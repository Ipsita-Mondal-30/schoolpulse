...
import { fetchEvents, SchoolEvent } from '@/app/actions';
import { getAllImportantDates, ImportantDate } from '@/lib/data';
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-start gap-4">
          ...
          <span className={`text-gray-400 transition-transform duration-200 mt-1 ${expanded ? 'rotate-180' : ''}`}>▾</span>
        </div>
        ...
        {/* Expanded details */}
        {expanded && (
          <div className="mt-4 pt-4 border-t border-gray-100 space-y-3 animate-[fadeIn_0.2s_ease]">
...
function EventsPageContent() {
  ...
  const [holidays, setHolidays] = useState<ImportantDate[]>([]);
  ...
  useEffect(() => {
    async function load() {
      const [evts, allDates] = await Promise.all([
        fetchEvents(),
        Promise.resolve(getAllImportantDates()),
      ]);
      setEvents(evts);
      setHolidays(allDates.filter(d => d.type === 'holiday'));
      setLoading(false);
    }
    load();
  }, []);
