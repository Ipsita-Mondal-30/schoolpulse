...
export default function HomeworkScanner({ onHomeworkScanned }: HomeworkScannerProps) {
  ...
  const [activeTab, setActiveTab] = useState<"photo" | "paste" | "manual">("photo");
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setStatusText("Processing and scanning worksheet image...");
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        if (result && result.subject) {
          ...
          setStatusText("✅ Image parsed and added successfully!");
        } else {
        ...
        setStatusText("⚠️ Vision parse failed, added as placeholder.");
      }
  ...
  const handlePasteAnalyze = async () => {
    if (!pasteText.trim()) {
      setStatusText("⚠️ Paste text details cannot be empty.");
      return;
    }
    ...
    try {
      ...
      if (result && result.subject) {
        onHomeworkScanned(result);
        setStatusText("✅ Text parsed and added successfully!");
        setPasteText("");
        setTimeout(() => {
          setStatusText("");
          setIsOpen(false);
      ...
      console.error(err);
      setStatusText("❌ Parsing failed. Please enter manually.");
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">
              Instantly scan worksheet photos, paste WhatsApp text, or add manual notes.
            </p>
          </div>
        </div>
        ...
        <div className="mt-3 p-5 bg-white rounded-3xl border border-gray-150 shadow-sm space-y-5">
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl border border-gray-200">
            <button
              ...
              className={`flex-1 text-xs py-2 px-1.5 font-bold rounded-lg transition-all ${
                activeTab === "photo"
          ...
          {activeTab === "photo" && (
            <div className="space-y-4">
              ...
              <button
                ...
                disabled={dailyUploads.length >= 5}
                className={`w-full py-5 border-2 border-dashed rounded-2xl text-xs font-bold flex items-center justify-center gap-2 transition-all ${
          ...
          {activeTab === "paste" && (
            <div className="space-y-3">
              <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider">
                Paste school WhatsApp diary text
              </label>
              <textarea
                value={pasteText}
                onChange={(e) => setPasteText(e.target.value)}
                placeholder="Paste raw WhatsApp text diary here... e.g. Mathematics: Chapter 2. Shapes. HW: Complete exercises 1-5 by Monday."
                rows={4}
